...
## R Code
```{r}
make_dummies <- function(data, columns) {
  for (column in columns) {
    values <- factor(data[[column]])
    for (level in levels(values)[-1]) {
      data[[paste0(column, "_", level)]] <- as.integer(values == level)
    }
    data[[column]] <- NULL
  }
  data
...
categorical_vars <- c("day_of_week", "geographic_region", "request_complexity", "cdn_usage")
gamma_server_encoded <- make_dummies(gamma_server, categorical_vars)
...
Why does this help keep the expected response $\mu_i$ positive?
::::
:::
...
If $\log(\mu_i) = \eta_i$, then $\mu_i = \exp(\eta_i)$. The exponential function is always positive, so the fitted mean response is always greater than zero, no matter what value the linear predictor takes.
...
The chapter states that Gamma regression often assumes
...
This variance relationship means that observations with larger expected response times also have larger variance. In practical terms, requests expected to be slower may also be more variable, which matches many cost, duration, and latency datasets.
...
**B.** It is the linear predictor $\eta_i = \beta_0 + \beta_1 x_{i1} + \cdots + \beta_p x_{ip}$.
...
The systematic component is the linear combination of predictors. In this chapter, variables such as `concurrent_users`, `database_queries`, `cache_hit_rate`, `server_load`, and the dummy variables for categorical predictors enter the model through this linear predictor.
...
Suppose a Gamma regression model has a coefficient of $\hat{\beta} = 0.20$ for `server_load`. Interpret this coefficient on the original response scale, holding other variables fixed.
::::
:::
...
Because the model uses a log link, exponentiating the coefficient gives the multiplicative change in the expected response. Here, $\exp(0.20) \approx 1.22$, so a one-unit increase in `server_load` is associated with about a 22% increase in expected response time, holding other variables fixed.
...
The coefficient for `request_complexity_Simple` compares simple requests to complex requests, holding the other predictors fixed. After exponentiating the coefficient, the result describes the multiplicative change in expected response time for simple requests relative to complex requests.
...
In this problem, you will investigate a dataset of food delivery orders and the time it took for each order to arrive.
Each order includes information about the trip distance, order size, restaurant conditions, time of day, traffic, weather, weekend status, driver experience, and total delivery time. These variables are stored as `distance_km`, `order_items`, `restaurant_busyness`, `time_of_day`, `traffic_level`, `weather`, `is_weekend`, `driver_experience_months`, and `delivery_time_minutes`, respectively. Below is a full breakdown of the variables:

| **Variable Name** | **Description** |
|-------------------|-----------------|
| **delivery_time_minutes** | The total delivery time in minutes. Float, strictly positive (e.g. 18.4, 42.7, etc.). |
| **distance_km** | The delivery distance from the restaurant to the customer in kilometers. Float (e.g. 2.5, 8.1, etc.). |
| **order_items** | The number of items in the food order. Integer (e.g. 1, 4, etc.). |
| **restaurant_busyness** | A restaurant busyness score from 0 to 100. Float (e.g. 35.2, 78.5, etc.). |
| **time_of_day** | The hour when the order was placed, using a 24-hour clock. Float (e.g. 12.5, 18.75, etc.). |
| **traffic_level** | The traffic level during the order. Categorical: `Low`, `Medium`, or `High`. |
| **weather** | The weather during the order. Categorical: `Clear`, `Light_Rain`, or `Heavy_Rain`. |
| **is_weekend** | Whether the order was placed on a weekend. Categorical: `Yes` or `No`. |
| **driver_experience_months** | The driver's experience in months. Float (e.g. 3.5, 24.0, etc.). |

A food delivery company wants to understand what kinds of orders tend to take longer. They are especially interested in whether distance, traffic, weather, restaurant busyness, and driver experience can help explain the expected delivery time.

To answer this, the questions below will guide you through a Gamma regression analysis on this data. Specifically, by the end of these questions, you will be able to answer the following concrete questions:
1. Which order conditions are associated with longer expected delivery times?
2. Why is Gamma regression with a log link a reasonable model for this outcome?
3. What delivery time should we expect for a specific new order?
4. How should these results be summarized for a delivery operations team?

### A. Data Collection and Wrangling {.unnumbered}

1. Read in the data from `problem-1-data.csv`, checking for any missing entries.
2. Drop rows with missing entries.
3. Convert `traffic_level`, `weather`, and `is_weekend` to categorical variables with sensible reference levels.
4. Confirm that `delivery_time_minutes` is strictly positive.
...
data_path <- "./04-gamma_files/simulations/problem-1-data.csv"
if (!file.exists(data_path)) {
  data_path <- "../docs/book/04-gamma_files/simulations/problem-1-data.csv"
}
...
summary(df$delivery_time_minutes)
sum(df$delivery_time_minutes <= 0)
...
data_path = Path("./04-gamma_files/simulations/problem-1-data.csv")
if not data_path.exists():
    data_path = Path("../docs/book/04-gamma_files/simulations/problem-1-data.csv")
...
df = df.dropna().copy()
df["traffic_level"] = pd.Categorical(df["traffic_level"], categories=traffic_levels)
df["weather"] = pd.Categorical(df["weather"], categories=weather_levels)
df["is_weekend"] = pd.Categorical(df["is_weekend"], categories=["No", "Yes"])

print(df["delivery_time_minutes"].describe())
print((df["delivery_time_minutes"] <= 0).sum())
print(df["traffic_level"].value_counts().reindex(traffic_levels))
print(df["weather"].value_counts().reindex(weather_levels))
```

:::
There are a small number of missing values in `restaurant_busyness` and `driver_experience_months`, so dropping incomplete rows is reasonable here. The response variable is strictly positive and right-skewed, which is consistent with the use of Gamma regression.
</details>

### B. Exploratory Data Analysis {.unnumbered}

1. Plot the distribution of `delivery_time_minutes`. Does it look right-skewed?
2. Plot `delivery_time_minutes` against `distance_km`. Does the average delivery time tend to increase with distance?
3. Plot `delivery_time_minutes` by `traffic_level`.
4. Plot mean delivery time across binned values of `restaurant_busyness`, separated by `weather`.
...
## **`R` Code**
```{r}
ggplot(df, aes(x = delivery_time_minutes)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "white") +
  theme_minimal() +
  labs(x = "Delivery time", y = "Count", title = "Distribution of delivery time")

ggplot(df, aes(x = distance_km, y = delivery_time_minutes)) +
  geom_point(alpha = 0.35) +
  geom_smooth(se = FALSE, color = "darkorange") +
  theme_minimal() +
  labs(x = "Distance, km", y = "Delivery time")

ggplot(df, aes(x = traffic_level, y = delivery_time_minutes, fill = traffic_level)) +
  geom_boxplot(alpha = 0.7) +
  theme_minimal() +
  labs(x = "Traffic level", y = "Delivery time")
...
distance_bins = pd.cut(df["distance_km"], bins=np.linspace(df["distance_km"].min(), df["distance_km"].max(), 11))
distance_means = df.groupby(distance_bins, observed=True)["delivery_time_minutes"].mean()
distance_centers = [interval.mid for interval in distance_means.index]

plt.figure()
plt.scatter(df["distance_km"], df["delivery_time_minutes"], alpha=0.35)
...
traffic_groups = [
    df.loc[df["traffic_level"] == level, "delivery_time_minutes"]
...
busyness_bins = pd.cut(df["restaurant_busyness"], bins=np.arange(0, 111, 10))
...
:::
The response is right-skewed, with many moderate delivery times and a smaller number of long deliveries. Delivery time generally increases with distance, is higher under heavier traffic, and tends to rise when restaurants are busier. Rainy weather also appears to shift delivery times upward.
</details>

### C. Data Modeling {.unnumbered}

1. Set up a Gamma regression model with `delivery_time_minutes` as the response.
2. Use a log link and include all remaining variables as predictors.
3. Use `Low` traffic, `Clear` weather, and non-weekend orders as reference levels.
4. Explain why this model is appropriate for the delivery-time outcome.
...
## **`R` Code**
```{r}
model_formula <- delivery_time_minutes ~ distance_km + order_items +
...
:::
Gamma regression is appropriate because `delivery_time_minutes` is continuous, strictly positive, and right-skewed. The log link also ensures that fitted expected delivery times remain positive and lets us interpret coefficients multiplicatively.
</details>

### D. Estimation {.unnumbered}

1. Fit the Gamma regression model using the formula above.
2. Display the model summary.
3. Exponentiate the coefficients so they can be interpreted as multiplicative effects on expected delivery time.
4. Which predictors appear to be most important?
...
coef_table <- tibble(
  term = names(coef(gamma_model)),
  multiplier = exp(coef(gamma_model)),
...
coef_table = pd.DataFrame({
    "term": gamma_model.params.index,
    "multiplier": np.exp(gamma_model.params),
    "percent_change": 100 * (np.exp(gamma_model.params) - 1)
})
...
:::
Longer distance, more restaurant busyness, high traffic, and rainy weather are associated with longer expected delivery times. More driver experience is associated with somewhat shorter expected delivery times. These effects are multiplicative because the model uses a log link.
</details>

### E. Goodness of Fit {.unnumbered}

1. Plot deviance residuals against fitted values.
2. Report the null deviance, residual deviance, and AIC for the fitted model.
3. Fit a simpler model using only `distance_km` and `order_items`.
4. Compare the full model to the simpler model using AIC and residual deviance.
...
df_diag <- tibble(
  ...
  deviance_resid = residuals(gamma_model, type = "deviance")
)

ggplot(df_diag, aes(x = fitted, y = deviance_resid)) +
...
fit_stats <- tibble(
  Model = c("Simple", "Full"),
  Null_Deviance = c(simple_model$null.deviance, gamma_model$null.deviance),
  Residual_Deviance = c(deviance(simple_model), deviance(gamma_model)),
  AIC = c(AIC(simple_model), AIC(gamma_model))
) %>%
...
```{python}
plt.figure()
plt.scatter(gamma_model.fittedvalues, gamma_model.resid_deviance, alpha=0.5)
...
fit_stats = pd.DataFrame({
    "Model": ["Simple", "Full"],
    "Null_Deviance": [simple_model.null_deviance, gamma_model.null_deviance],
    "Residual_Deviance": [simple_model.deviance, gamma_model.deviance],
    "AIC": [simple_model.aic, gamma_model.aic]
})

print(fit_stats.round(2))
```

:::
The deviance residuals should be centered around zero without a strong pattern. The full model should have lower AIC and residual deviance than the simple model, suggesting that traffic, weather, busyness, weekend status, and driver experience add useful information beyond distance and order size alone.
</details>

### F. Results and Storytelling {.unnumbered}

1. Interpret the coefficient for `distance_km` on the original response scale.
2. Interpret the coefficient for `traffic_levelHigh` relative to `traffic_levelLow`.
...
4. Returning to the original delivery question: what have we learned about which order conditions are associated with longer delivery times?
5. Write a short storytelling summary for a delivery operations team. Your summary should explain the main finding, one model limitation, and one practical implication in plain language.
...
```{r}
coef_table %>%
  filter(term %in% c("distance_km", "traffic_levelHigh", "weatherLight_Rain",
                     "restaurant_busyness", "driver_experience_months")) %>%
  print()
...
new_order <- data.frame(
  ...
  time_of_day = 18.5,
  traffic_level = factor("High", levels = levels(df$traffic_level)),
  weather = factor("Light_Rain", levels = levels(df$weather)),
  is_weekend = factor("Yes", levels = levels(df$is_weekend)),
  driver_experience_months = 12
)

predict(gamma_model, newdata = new_order, type = "response")
```

## **`Python` Code**
```{python}
print(coef_table[
    coef_table["term"].isin([
        "distance_km",
...
print(gamma_model.predict(new_order).round(1))
```

:::
The `distance_km` coefficient is positive, so each additional kilometer is associated with a multiplicative increase in expected delivery time, holding the other variables fixed. The `traffic_levelHigh` coefficient compares high-traffic orders to low-traffic orders; after exponentiating, a value above 1 means high traffic is associated with longer expected delivery times. Overall, the model suggests that delivery time is shaped most by practical delay sources: distance, restaurant busyness, traffic, weather, and driver experience.

A short storytelling summary could be: food delivery time is usually moderate, but it has a long right tail because some orders are delayed by distance, high traffic, busy restaurants, and rain. The Gamma regression model helps quantify these patterns, but it is still associational and does not prove that changing one condition would directly cause the exact predicted time to change. For a delivery operations team, the main implication is to plan extra capacity around high-traffic, rainy, and high-busyness periods, while using the model as a guide rather than a perfect clock.
</details>
